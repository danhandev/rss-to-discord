...
IT 트렌드를 따라가고 인사이트를 얻기 위해 기술 블로그와 공식 사이트를 주기적으로 열람하고 있습니다. 다만 사이트마다 링크를 눌러 들어가야 했고, 안 읽은 글이 어디서부터인지 매번 확인하는 일이 반복됐습니다. 이 두 가지를 덜어내고자 수집을 자동화하기로 했습니다.

팀 프로젝트에서 이미 Discord를 사용하고 있어 새로 열어야 할 앱이 늘지 않는다는 점이 컸습니다. 접근성이 가장 좋은 창구라고 판단해 이곳으로 글이 모이도록 연결했습니다.

개인용 도구인 만큼 인프라 없이 최대한 간단하게 두고 싶었습니다. GitHub Actions 위에서 동작하도록 만들고, 상태는 DB 없이 저장소의 `state.json` 한 파일로만 관리합니다.
