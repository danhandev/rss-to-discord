...
### weekly-tech — 기업 기술블로그

토스테크, 우아한형제들, 카카오, 뱅크샐러드, NAVER D2, LINE, 당근, 쿠팡, 컬리, 데브시스터즈, 쏘카, 하이퍼커넥트, 요즘IT

트래픽과 데이터 규모가 큰 서비스가 어떤 문제를 어떻게 풀었는지 보려고 모았습니다. 결제 및 정산을 다루는 토스테크와 뱅크샐러드는 특히 눈여겨보고 있습니다.

우아한형제들과 요즘IT는 원본 피드가 GitHub Actions에서 접근되지 않아 Google News 검색 피드를 경유하도록 했습니다. 이 두 곳은 링크가 Google News를 거치고 요약이 제공되지 않습니다.
