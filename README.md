...
구독하는 곳이 늘어날수록 RSS 리더에는 안 읽은 글만 쌓였고, 결국 아무것도 읽지 않게 됐습니다. 원인은 성격이 다른 글이 한 목록에 섞여 있다는 점이라고 판단했습니다. 그날 바로 봐야 의미가 있는 뉴스와, 주말에 몰아서 읽어도 충분한 기업 기술블로그가 같은 자리에서 경쟁하면 양쪽 다 밀리게 됩니다.

그래서 새로운 앱을 습관에 추가하는 대신, 이미 매일 열고 있는 Discord로 글을 밀어 넣기로 했습니다. 읽는 주기를 기준으로 채널을 나눠 급한 것은 바로 오게 하고 급하지 않은 것은 주 1회 한 메시지로 묶었습니다.

개인용 도구에 서버를 띄우고 관리할 이유가 없어 GitHub Actions 위에서 동작하도록 만들었습니다. 상태는 저장소의 `state.json` 한 파일에만 남기므로 별도의 DB도 두지 않았습니다.

<br>

## 2. 구독 중인 피드

실제로 연결해 둔 목록입니다. 채널을 나눈 기준이 곧 이 글들을 어떤 주기로 읽고 싶은지에 대한 판단입니다.
...
트래픽과 데이터 규모가 큰 서비스가 어떤 문제를 어떻게 풀었는지 보려고 모았습니다. 결제·정산을 다루는 토스테크, 카카오페이, 뱅크샐러드의 비중이 높습니다.
